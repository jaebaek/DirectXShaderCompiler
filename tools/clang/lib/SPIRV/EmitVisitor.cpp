...
bool EmitVisitor::visit(SpirvDebugDeclare *inst) {
  ...
  curInst.push_back(
      getOrAssignResultId<SpirvInstruction>(inst->getInstructionSet()));
  curInst.push_back(inst->getDebugOpcode());
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getDebugLocalVariable()));
  curInst.push_back(
      getOrAssignResultId<SpirvInstruction>(inst->getVariable()));
  curInst.push_back(getOrAssignResultId<SpirvInstruction>(inst->getDebugExpression()));
  finalizeInstruction(&mainBinary);
  return true;
}
