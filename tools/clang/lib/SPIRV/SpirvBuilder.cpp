...
SpirvDebugDeclare *SpirvBuilder::createDebugDeclare(SpirvDebugLocalVariable *dbgVar,
                                                    SpirvInstruction *var,
                                                    llvm::Optional<SpirvDebugExpression *> dbgExpr) {
  if (auto *localVar = dyn_cast<SpirvVariable>(var)) {
    auto *decl =
        new (context) SpirvDebugDeclare(dbgVar, var, dbgExpr.hasValue() ? dbgExpr.getValue() : getOrCreateNullDebugExpression());
    assert(!localVar->getDebugDeclare() && "Debug info for local variable already exists!");
  ...
  if (auto *param = dyn_cast<SpirvFunctionParameter>(var)) {
    auto *decl =
        new (context) SpirvDebugDeclare(dbgVar, var, dbgExpr.hasValue() ? dbgExpr.getValue() : getOrCreateNullDebugExpression());
    assert(!param->getDebugDeclare() && "Debug info for function param already exists!");
